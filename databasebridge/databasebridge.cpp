...
void DataBaseBridge::dropTable(const QString &table)
{
    // delete all data in the table and discard the table.
    if(db.tables().contains(table))
    {
        checkSuccess(query.exec(QString("DROP TABLE %1").arg(table)), tr("drop table"));
    }
    else
    {
...
//void DataBaseBridge::createTable(const QString &table, const QStringList& fields)
//{
//    not tested
//    if(db.tables().contains(table))
//    {
...
//}
void DataBaseBridge::createTable(const QString &table, const QString& fields)
{
    ...
    checkSuccess(query.exec(QString("create table %1 (%2)").arg(table).arg(fields)), tr("create table"));
...
void DataBaseBridge::insertItem(const QString& table, const QList<QVariant>& varList)
{
    ...
    checkSuccess(success, "insert");
}

void DataBaseBridge::checkSuccess(bool success, const QString& action)
{
    if(success)
    {
...
void DataBaseBridge::deleteItem(const QString& table, const QString& condition)
{
